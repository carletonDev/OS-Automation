...
public class ClassParser {
  ...
  public <T> Object[][] getTestngData(Class<?> type, Map<String, List<String>> dataMap)
  ...
  <T> List<T> getClassObjectList(Class<?> type, Map<String, List<String>> dataMap)
    ...
    try {
      ...
      if (dataNotAvailable) {
        throw new DataNotAvailableException("Data for the fields :" + notAvialableFieldData
            + "is not available in the provided map.");
      }
  ...
  public List<TestCase> getTestCaseFromMap(Map<String, List<String>> dataMap)
      throws DataNotAvailableException {

    boolean dataNotAvailable = false;
    List<String> notAvialableFieldData = new ArrayList<String>();
    List<TestCase> dataObject = new ArrayList<>();
    try {
      ...
      for (int i = 0; i < noOfDataSets; i++) {
        ...
        for (Field field : fields) {
          if (field.getModifiers() == Modifier.PUBLIC) {
            String fieldName = field.getName();
            if (dataMap.containsKey(fieldName) && !fieldName.equals("suite")) {
              List<String> dataValues = dataMap.get(fieldName);
              String dataValue = dataValues.get(i);
              this.setValue(field, testCase, dataValue);
            } else {
              dataNotAvailable = true;
              notAvialableFieldData.add(fieldName);
            }
  ...
  public List<TestCaseSteps> getTestCaseStepsFromMap(Map<String, List<String>> dataMap)
      throws DataNotAvailableException {

    boolean dataNotAvailable = false;
    List<String> notAvialableFieldData = new ArrayList<String>();
    List<TestCaseSteps> dataObject = new ArrayList<>();
    try {
      Field[] fields = TestCase.class.getDeclaredFields();
      List<String> tempKeys = new ArrayList<String>(dataMap.keySet());
      int noOfDataSets = dataMap.get(tempKeys.get(0)).size();
      for (int i = 0; i < noOfDataSets; i++) {
        ...
        for (Field field : fields) {
          if (field.getModifiers() == Modifier.PUBLIC) {
            String fieldName = field.getName();
            if (dataMap.containsKey(fieldName) && !fieldName.equals("testCase") && !fieldName
                .equals("steps")) {
              List<String> dataValues = dataMap.get(fieldName);
              String dataValue = dataValues.get(i);
              this.setValue(field, testCaseSteps, dataValue);
            } else {
              dataNotAvailable = true;
              notAvialableFieldData.add(fieldName);
            }
    ...
    try {
      ...
      if (fieldType == int.class || fieldType == Integer.class) {
        ...
        throw new ClassParserException(
            "Unable to convert the value:" + value + "to required class:" + fieldType);
      }
      field.set(obj, data);
    } catch (Exception e) {
      throw new ClassParserException(
